...
libcd.genGaussionHeatmap.argtypes = [c_int32,c_int32,c_int32,c_int32,array_1d_double]


def genGaussionHeatmap(height,width,x,y):
    out_array = np.zeros(shape = (height,width)).reshape((-1,))
    libcd.genGaussionHeatmap(height,width,x,y,out_array)
    out_array = out_array.reshape((height,width))
    return out_array
if __name__ == "__main__":
    # out_array = np.zeros(shape = (512,512)).reshape((-1,))
    out_array = genGaussionHeatmap(512,512,20,20)

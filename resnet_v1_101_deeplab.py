...
#                                   stride=(2, 2), pool_type='max')
    ...
    def get_body(self,data, heatmaplabel,partaffinityglabel,loss_mask,istrain):
        sym_body = self.get_resnet_conv(data)
        heat_p = mx.symbol.Convolution(data=sym_body, kernel=(3, 3),
                                  pad=(1, 1), num_filter=19, name="conv_last_heat",
                        workspace=self.workspace)
        paf_p = mx.symbol.Convolution(data=sym_body, kernel=(3,3),
                                  pad=(1, 1), num_filter=19 * 2, name="conv_last_paf",
                        workspace=self.workspace)

        conv4_3_CPM = mx.symbol.Convolution(name='conv4_3_CPM', data=sym_body, num_filter=256, pad=(1, 1), kernel=(3, 3),
        ...
        heatmaps_predicts = [heat_p,conv5_5_CPM_L2, Mconv7_stage2_L2, Mconv7_stage3_L2, Mconv7_stage4_L2,
                             Mconv7_stage5_L2, Mconv7_stage6_L2]
        pafmaps_predicts = [paf_p,conv5_5_CPM_L1, Mconv7_stage2_L1, Mconv7_stage3_L1, Mconv7_stage4_L1,
        ...
        if istrain:
            # losses = []
            #
            # for p in heatmaps_predicts:
            #     losses.append(mx.sym.broadcast_mul( mx.sym.square(p - heatmaplabel) , loss_mask)/(46*46))
            # for p in pafmaps_predicts:
            #     losses.append(mx.sym.broadcast_mul( mx.sym.square(p - partaffinityglabel), loss_mask)/(46*46*2))
            # return mx.sym.Group([mx.sym.MakeLoss(x) for x in losses])
            heat_p = mx.symbol.sigmoid(heat_p)
            loss1 = mx.sym.broadcast_mul((heatmaplabel * mx.symbol.log(heat_p + 1e-12) + (1 - heatmaplabel) * mx.symbol.log(
                1 - heat_p + 1e-12)) ,loss_mask) * -1
...
def get_symbol(is_train = True,numberofparts = 19,numberoflinks = 19):
    ...
    loss_mask = mx.sym.Variable("loss_mask")

    # heatmaplabel,partaffinityglabel,heatweight,vecweight = slice_label(label,numberofparts,numberoflinks)
    Sym = resnet_v1_101_deeplab()
    r = Sym.get_body(data,heatmaplabel,partaffinityglabel,loss_mask,is_train)
    return  r

...
def get_resnet_fpn_rpn(numberofparts,numberoflinks):
    ...
    for stride in config.RPN_FEAT_STRIDE:
        heatmap_conv = mx.symbol.Convolution(data=conv_fpn_feat['stride%s'%stride],
                                         kernel=(3, 3), pad=(1, 1),
                                         num_filter=512)
        heatmap_relu = mx.symbol.Activation(data=heatmap_conv, act_type="relu", name="heatmap_relu")
        ...
        # prepare rpn data
        heatmap_score_reshape = mx.symbol.Reshape(data=heatmap_score,
                                                  shape=(0,  -1),
                                                  name="heatmap_score_reshape_stride%s" % stride)

        heatmap_score_list.append(heatmap_score_reshape)

        pafmap_conv = mx.symbol.Convolution(data=conv_fpn_feat['stride%s'%stride],
                                         kernel=(3, 3), pad=(1, 1),
                                         num_filter=512)
        pafmap_relu = mx.symbol.Activation(data=pafmap_conv, act_type="relu", name="pafmap_relu")
        pafmap_score = mx.symbol.Convolution(data=pafmap_relu,
                                              kernel=(1, 1), pad=(0, 0),
                                              num_filter=numberoflinks*2,
                                              name="pafmap_score_stride%s" % stride)
        # prepare rpn data
        pafmap_score_reshape = mx.symbol.Reshape(data=pafmap_score,
                                                  shape=(0,  -1),
                                                  name="pafmap_score_reshape_stride%s" % stride)

        pafmap_score_list.append(pafmap_score_reshape)
    # concat output of each level
    heatmap_score_concat = mx.symbol.concat(*heatmap_score_list, dim=1)
    pafmap_score_concat = mx.symbol.concat(*pafmap_score_list, dim=1)

    heatmaplabel = mx.symbol.Variable(name = "heatmaplabel")
    partaffinityglabel = mx.symbol.Variable(name = "partaffinityglabel")
